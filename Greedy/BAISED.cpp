...
// C++ template to print pair container elements
ostream& operator<<(ostream& os, const pair<long long int, string>& p)
{
	os << p.first << " " << p.second;
	return os;
}
...
int main()
{
	...
	while (t--) {
		int participants;
		cin >> participants;
		vector<pair<long long int, string>> ranklist;
		...
		for (int i = 0; i < participants; i++)
			badness += abs(i + 1 - ranklist[i].first);
